...
#define is_play_cali()                                                         \
  (is_play == CALI_BEGIN || is_play == CALI_MIDDLE_TIME ||                     \
   is_play == CALI_GIMBAL || is_play == CALI_IMU || is_play == CALI_CHASSIS)

#define cali_buzzer_begin() buzzer_on(50, 10000)  // 蜂鸣器的设置频率和强度
#define cali_buzzer_middle() buzzer_on(20, 10000) // 蜂鸣器的设置频率和强度
#define cali_buzzer_gimbal()                                                   \
  buzzer_on(30, 19999) // 当云台在校准,蜂鸣器的设置频率和强度
#define cali_buzzer_imu()                                                      \
  buzzer_on(60, 19999) // 当imu在校准,蜂鸣器的设置频率和强度
#define cali_buzzer_chassis()                                                  \
  buzzer_on(100, 19999)                // 当底盘在校准,蜂鸣器的设置频率和强度
